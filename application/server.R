shinyServer(function(input, output) {
    ...
    newdata <- reactive({
        ...
        data.frame(x = ceiling((as.numeric(clean()$results[[1]]$geometry$location$lat) - centre[2])/met_deg[2]),
                   y = ceiling((as.numeric(clean()$results[[1]]$geometry$location$lng) - centre[1])/met_deg[1]),
    ...
    output$map <- renderLeaflet({
        ...
        leaflet() %>% setView(lat = clean()$results[[1]]$geometry$location$lat, 
                              lng = clean()$results[[1]]$geometry$location$lng, zoom = 13) %>% 
            addMarkers(lat = as.numeric(clean()$results[[1]]$geometry$location$lat), 
                      lng = as.numeric(clean()$results[[1]]$geometry$location$lng), 
